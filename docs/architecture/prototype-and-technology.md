...
- **0a smoke**: 10³-10⁴ vectors including hand-built edge cases, byte-identical canonical transcripts and roots across x86-64/ARM64/RISC-V. Status: closed. `conformance/` contains the immutable 51-check constitutional seed corpus, the seeded 1136-check expansion tier, the Odin generator, and an independent Python checker derived from `canonical-encoding.md` with no shared code. The seed corpus serves as historical evidence; only the expansion tier grows. The v1 authenticated tree has two tiers. A hand-derived constitutional tier of 16 checks pins the construction. A generator-produced expansion tier covers occupancy, bit boundaries, proofs, bounded updates and rejections. A hand-derived Transition 1 tier contains 3 scenarios that pin what the kernel commits under `blockmon/world/v1`. The v0 corpora are archived against pinned digests, not reproduced. `conformance/cross-target.sh` emits the v1 tree expansion tier on darwin arm64, linux arm64, linux amd64, and linux riscv64. Every leg must be byte-identical to the native run and the committed vectors. Archived v0 corpora are digest-verified, never regenerated. Because it requires docker and a riscv64 container, it remains off the per-change CI path. The intended shape is a scheduled or manually dispatched job, keeping the fast constitutional gates on every push and pull request.
- **0b hardening**: ≥10⁶ fuzzed transitions, same criterion; permanent CI gate. Status: closed. Property/fuzz harness at `conformance/fuzz` (`just g0b`; raw-struct generators, independent model oracle, canonical-bytes equality, seed-reproducible); >8M transitions across four seeds, including the pinned default 0xB10C0DE5EED (12166583181037, shared by `just g0b` and the bare binary), pass with zero counterexamples. Every push to main and every pull request triggers the permanent CI gate in `.github/workflows/conformance.yml`. This runs the lint gate, the kernel and canonical suites, archived g0a corpora against their pinned digests, g0b at full budget, the two v1 tiers (`tree-v1`, `t1-v1`) with a byte-identical corpus check, and g0c plus `tree-v1-rust` on their own toolchain path. The harness carries the authenticated commitment through the bounded update path and compares it against a full derivation, budgeted per phase: 500 steps locally and 20000 in CI. The run summary reports the count reached.
- **0c independent implementation**: a second kernel implementation built from the written spec (no reference-source reuse) matches canonical vectors. Status: closed. The cleanroom Rust kernel in `conformance/g0c/` reproduces both corpora at 1187/1187 from the written spec alone, deriving no value from a vector. This exercise yielded three corrections to `canonical-encoding.md`: assigned reject discriminants, the definition of valid kernel input, and the extant == created clause. `REPORT.md` records each isolated session verbatim. A second cleanroom at `conformance/tree-v1/impl` was built from §7 alone under the same discipline and reproduces both v1 tree tiers. It yielded further corrections to `canonical-encoding.md`, among them the record identifier binding at the proof boundary, and now runs as a standing gate rather than as a one-off exercise.
